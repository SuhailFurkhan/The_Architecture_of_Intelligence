...
if "auto_select_result" not in st.session_state:
    st.session_state.auto_select_result = None   # filled on first key check
if "llm_provider" not in st.session_state:
    # Best-effort guess from Keys.env presence (no live check yet at startup)
    if ENV_KEYS.get("ANTHROPIC_API_KEY"):
        st.session_state.llm_provider = "anthropic"
    elif ENV_KEYS.get("OPENAI_API_KEY"):
...
def initialize_llm_assistant():
    """
    Initialize the LLM assistant using the auto-selected provider.
    Keys.env is always read fresh — session state key is only used if the
    user explicitly overrode it via the sidebar.
    """
    provider = st.session_state.llm_provider

    # Only forward a manual override key if it differs from Keys.env
    env_key     = ENV_KEYS.get("ANTHROPIC_API_KEY" if provider == "anthropic" else "OPENAI_API_KEY", "")
    session_key = st.session_state.llm_api_key
    override    = session_key if (session_key and session_key != env_key) else None

    st.session_state.llm_assistant = LLMAssistant(provider=provider, api_key=override)
...
# =========================================================================
    ...
    with st.sidebar:
        ...
        col_auto, col_reset = st.sidebar.columns([3, 1])
        with col_auto:
            auto_btn = st.button(
                "🔍 Auto-Select Provider",
                use_container_width=True,
                key="auto_select_btn",
                help="Tests both Anthropic and OpenAI keys and picks the best one",
            )
        with col_reset:
            if st.button("↺", use_container_width=True, key="reset_auto_btn",
                         help="Clear result and re-run"):
                st.session_state.auto_select_result = None
                st.session_state.llm_assistant = None
                st.rerun()

        if auto_btn:
            with st.spinner("Checking keys and selecting provider..."):
                result = auto_select_provider()
                st.session_state.auto_select_result = result
                # Apply the selected provider immediately
                selected = result["provider"]
                if selected != st.session_state.llm_provider:
                    st.session_state.llm_provider    = selected
                    st.session_state.llm_assistant   = None
                    st.session_state.llm_api_key     = ENV_KEYS.get(
                        "ANTHROPIC_API_KEY" if selected == "anthropic" else "OPENAI_API_KEY", ""
                    )

        result = st.session_state.auto_select_result
        if result is None:
            st.caption("Click **Auto-Select Provider** to test your keys.")
        else:
            # Show reason banner
            provider_icons = {"anthropic": "🔷", "openai": "🟢", "mock": "🧪"}
            icon = provider_icons.get(result["provider"], "❓")
            if result["provider"] == "mock":
                st.warning(f"{icon} **{result['reason']}**")
            else:
                st.success(
                    f"{icon} **Active: {result['provider'].title()}**  \n"
                    f"Model: `{result['model']}`  \n"
                    f"_{result['reason']}_"
                )

            # Per-key status breakdown
            with st.expander("🔑 Key Status Detail", expanded=False):
                for key_name, s in [
                    ("ANTHROPIC_API_KEY", result["anthropic"]),
                    ("OPENAI_API_KEY",    result["openai"]),
                ]:
                    label = "Anthropic" if "ANTHROPIC" in key_name else "OpenAI"
                    if not s.present:
                        st.warning(f"**{label}** — not configured", icon="➖")
                    elif s.valid:
                        st.success(
                            f"**{label}** ✅  \n"
                            f"`{s.key_preview}` → `{s.model}`",
                            icon="✅",
                        )
                    else:
                        st.error(
        ...
        # Set selectbox index to match auto-selected provider
        ...
        if provider != "mock":
            ...
            if current_key:
                st.caption(f"Active key: `{current_key[:8]}...{current_key[-4:]}`")
            ...
            with st.expander("🔑 Override API Key"):
                ...
                if manual_key and manual_key != st.session_state.llm_api_key:
                    st.session_state.llm_api_key      = manual_key
                    st.session_state.llm_assistant    = None
                    st.session_state.auto_select_result = None  # force recheck

            if not st.session_state.llm_api_key:
                st.warning("⚠️ No API key. Add to Keys.env or enter above.")
        else:
...
# def initialize_llm_assistant():
#     """
#     Initialize or reinitialize the LLM assistant.
#
#     Does NOT pass api_key from session state — LLMAssistant reads Keys.env
#     fresh on every init, so it always picks up the latest key value.
#     The session state key is only used when the user has manually overridden
#     it via the sidebar input (detected by checking against ENV_KEYS).
#     """
#     provider = st.session_state.llm_provider
#
#     # Only forward the session key if the user explicitly overrode it
#     # (i.e. it differs from what Keys.env contains right now)
#     env_key = ENV_KEYS.get("ANTHROPIC_API_KEY" if provider == "anthropic" else "OPENAI_API_KEY", "")
#     session_key = st.session_state.llm_api_key
#     override_key = session_key if (session_key and session_key != env_key) else None
#
#     st.session_state.llm_assistant = LLMAssistant(
#         provider=provider,
#         api_key=override_key,   # None → LLMAssistant reads from Keys.env
#     )
#
#
# # =============================================================================
...
#                     st.session_state.llm_assistant = None
#                     st.session_state.key_statuses = None   # force recheck
#
#             if not st.session_state.llm_api_key:
...
# #     st.session_state.key_statuses = None   # None = not yet checked
# #
# #
# # # =============================================================================
# # # CUSTOM CSS
...
# #
# #         col_check, col_clear = st.sidebar.columns([3, 1])
# #         with col_check:
# #             check_btn = st.button(
# #                 "🔍 Check Keys",
# #                 use_container_width=True,
...
# #             )
# #         with col_clear:
# #             if st.button("↺", use_container_width=True, key="clear_key_cache_btn",
# #                          help="Clear cached results"):
# #                 st.session_state.key_statuses = None
# #                 st.rerun()
# #
# #         if check_btn:
# #             with st.spinner("Validating keys..."):
# #                 st.session_state.key_statuses = validate_all_keys()
# #
# #         statuses: dict = st.session_state.key_statuses
# #
# #         if statuses is None:
# #             st.caption("Click **Check Keys** to validate your API keys.")
# #         else:
# #             PROVIDER_LABELS = {
# #                 "anthropic":   "Anthropic Claude",
...
# #                 "openai":      "OpenAI",
# #             }
# #             for key_name, s in statuses.items():
...
# #
# #                 elif s.valid:
# #                     detail = s.hf_username or s.model or "active"
# #                     st.success(
# #                         f"**{label}** — ✅ valid  \n"
# #                         f"`{s.key_preview}`  \n"
# #                         f"Model / User: `{detail}`",
